    ...
    static parse(persianDateTimeInString: string, dateSeperatorPattern: string = '\/|-') {
      ...
      if (persianDateTimeInString.indexOf(':') > -1) // رشته ورودی شامل ساعت نیز هست
      {
        ...
        if (persianDateTimeInString.indexOf(':') != persianDateTimeInString.lastIndexOf(':')) {
          ...
          miliSecond = miliSecondMatch == null || miliSecondMatch.length <= 0 ? "0" : miliSecondMatch[0].match(/:\d{1,4}-/img)[0].replace(/\D+/img, '');
        }
      }
      ...
      // بررسی سحت داده های ساعت
      if (numericHour > 23 || numericHour < 0 ||
          numericMinute > 59 || numericMinute < 0 ||
          numericSecond > 59 || numericSecond < 0 ||
          numericMiliSecond > 59 || numericMiliSecond < 0) 
      {
        throw new Error("ساعت وارد شده معتبر نمی باشد");
      }
      ...
      // بررسی صحت داده های تاریخ
      if (numericYear < 0 ||
        numericMonth > 12 || numericMonth < 0 ||
        numericDay < 0 ||
        (numericDay > 31 && numericMonth <= 6) ||
        (numericDay > 30 && numericMonth <= 12 && numericMonth > 6)) 
      {
        throw new Error("تاریخ وارد شده معتبر نمی باشد");
      }
